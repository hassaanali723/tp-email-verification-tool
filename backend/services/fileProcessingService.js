...
class FileProcessingService {
    ...
    async processFile(fileData) {
        ...
        try {
            ...
            const { emails, processedRows } = await this._extractEmails(fileData, fileRecord._id);
            ...
            // If file record exists, update its status to failed
            if (fileRecord) {
                await File.findByIdAndUpdate(fileRecord._id, {
                    status: 'failed'
                });
        ...
        return new Promise((resolve, reject) => {
            ...
            createReadStream(filePath)
                ...
                .on('data', async (row) => {
                    ...
                    if (potentialEmail && emailRegex.test(potentialEmail)) {
    ...
    _findEmailInRow(row) {
        ...
        // If no email column found, check all columns for email format
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
